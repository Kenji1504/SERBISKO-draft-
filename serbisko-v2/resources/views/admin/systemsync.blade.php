...
@section('content')
    <div x-data="{ syncing: false }" class="w-full h-[100px] bg-[#F7FBF9]/50 rounded-[10px] shadow-[0_3px_3px_0_rgba(0,0,0,0.25)] flex items-center px-12 justify-between">
        ...
        <div class="flex items-center gap-4"> 
            <form action="{{ route('admin.sync.perform') }}" method="POST" @submit="syncing = true" class="flex items-center gap-6">
                @csrf
                <input type="hidden" name="target_sy" value="{{ $settings->active_school_year }}">
                <button type="submit" 
                        :disabled="syncing"
                        class="bg-[#00923F] hover:bg-[#04578F] text-white px-6 py-2 rounded-lg flex items-center gap-3 shadow-md transition-all active:scale-95 group disabled:opacity-70 disabled:cursor-not-allowed mt-4">
    ...
    <div class="w-full flex items-center justify-between my-10"> 
        <div class="flex items-center gap-10">
            {{-- Dynamic Edit Form URL --}}
            <a href="{{ $settings->edit_form_url ?? '#' }}" 
                target="_blank" 
                class="flex items-center gap-2 text-[#00923F] font-bold border-b-2 text-md border-[#00923F] pb-0.5 hover:opacity-30 transition-all">
            ...
            {{-- Dynamic Spreadsheet Link --}}
            {{-- Logic: If ID exists, build the full Google Sheets URL --}}
            @php
                $spreadsheetUrl = $settings->active_spreadsheet_id 
                    ? "https://docs.google.com/spreadsheets/d/" . $settings->active_spreadsheet_id 
                    : "#";
            @endphp
        ...
        {{-- Dynamic Copy URL Section --}}
        <div x-data="{ publicUrl: '{{ $settings->public_form_url ?? '' }}' }" class="flex items-center gap-2">
            <input type="text" 
                :value="publicUrl" 
                readonly 
                placeholder="{{ $settings->public_form_url ?? 'No URL configured' }}"
                class="w-[400px] border rounded-lg px-3 py-2 bg-gray-50 text-gray-500 text-sm focus:outline-none italic">
            
            <button @click="if(publicUrl) { navigator.clipboard.writeText(publicUrl); alert('Link copied!'); } else { alert('No link to copy!'); }" 
                    class="bg-green-700 hover:bg-green-800 text-white px-4 py-2 rounded-lg flex items-center gap-2 shrink-0 transition-colors">
    ...
    <div class="grid grid-cols-1 md:grid-cols-12 gap-6 mt-6">
        <div class="overflow-y-auto custom-scrollbar md:col-span-12 pr-4 relative" style="max-height: 320px;">
            ...
            <table class="min-w-full table-auto border-collapse">
                <thead>
                    <tr class="border-y border-gray-500 sticky top-10 bg-white z-10">
                        ...
                        <th class="py-2 text-[#004225] font-bold text-xs uppercase tracking-widest text-center">School Year</th>
                ...
                <tbody>
                    @forelse ($syncHistory as $item)
                        <tr class="border-b border-gray-300 text-center hover:bg-white/50 transition-colors">
                            ...
                            <td class="py-4 text-sm font-bold text-[#00923F]">{{ $item->school_year }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="py-10 text-center text-gray-400 italic">No sync history found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
